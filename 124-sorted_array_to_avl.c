...
avl_t *sorted_array_to_avl(int *array, size_t size)
{
	...
	array_sort(&tree, array, -1, middle);
	array_sort(&tree, array, middle, size);

	return (tree);
}

/**
 * array_sort - helper that builds an AVL tree from an array
 * @root: double pointer to the root node of the subtree
 * @array: a pointer to the first element of the array to be converted
 * @lo: lower bound index
 * @hi: upper bound index
 */
void array_sort(avl_t **root, int *array, size_t lo, size_t hi)
{
	avl_t *new = NULL;
	size_t middle;
	...
	if (hi - lo > 1)
	{
		...
		if (array[middle] > (*root)->n)
			...
			(*root)->left = new;
		array_sort(&new, array, lo, middle);
		array_sort(&new, array, middle, hi);
	}
}
